...
#Key to open AI image and text generation
...
def index():
    ...
    #This is the mechanism for identifying directories to consider deleting.
    ...
    #This calls a method that deletes any directories more than a day old
    deleteOldDirectories(userDirectoryHeader = userDirectoryHeader, user_id = user_id)

    #These flags determine whether it makes new content or just uses default content
    generate_new_story = True
    generate_new_images = True
#    generate_new_story = False
#    generate_new_images = False

    #This makes a new directory to hold this user's content
    ...
    if request.method == "POST":
        ...
        #There used to be a style input, but most of them looked worse than claymation (especially re: background subtraction), so I stuck with that.
...
#        setting_list = ["rain forest", "ocean", "beach", "desert", "stream", "grotto"]
        ...
        character_list1 = ["A boy named Bob", "a nonbinary person named Kori", "a girl named Dominique", "a girl named Jasmine", "a boy named Jalen, who has dark hair", "a boy named Clark, who uses a wheelchair", "a boy named Malik, who likes pizza", "a girl named Min", "a girl named Mei", "a boy named Mako", "a boy named Ajay", "a girl named Divya", "a girl named Ayla"]
    ...
    for subdir, dirs, files in os.walk("static/"):
        for dir in dirs:
            if str(dir).startswith(userDirectoryHeader) and not dir == user_id:
                ...
                if timeDifference.seconds > 24*60*60:
